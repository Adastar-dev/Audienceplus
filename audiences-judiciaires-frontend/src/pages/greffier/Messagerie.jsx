...
import { listerJuges, listerProcureurs, listerAvocats } from '../../services/api/utilisateurs'
import { listerConversation, envoyerMessage } from '../../services/api/messages'
import { useAuth } from '../../context/AuthContext'
import { formatMessageDate } from '../../utils/formatMessageDate'

// Le greffier échange avec les autres acteurs du tribunal : juges et
// procureurs, ainsi qu'avec les avocats pour la coordination administrative
// sur leurs dossiers. On fusionne les trois listes dans un seul sélecteur de
...
export default function Messagerie() {
  ...
  useEffect(() => {
    Promise.all([listerJuges(), listerProcureurs(), listerAvocats()])
      .then(([juges, procureurs, avocats]) => {
        const liste = [
          ...juges.map((j) => ({ ...j, role: 'Juge' })),
          ...procureurs.map((p) => ({ ...p, role: 'Procureur' })),
          ...avocats.map((a) => ({ ...a, role: 'Avocat' })),
        ]
        setContacts(liste)
        if (liste.length > 0) setIdContact(liste[0].id_utilisateur)
      })
      .catch(() => setErreur('Impossible de charger la liste des contacts.'))
  ...
  return (
    <div>
      <h1 className="font-display text-2xl text-navy-900">Messagerie</h1>
      <p className="text-sm text-slate-600 mt-1">Conversation privée avec un juge, un procureur ou un avocat.</p>

      {erreur && <p className="bg-danger-100 text-danger-700 text-sm rounded px-3 py-2 mt-4">{erreur}</p>}

      {contacts.length === 0 && !erreur ? (
        <p className="text-sm text-slate-400 mt-6">Aucun contact disponible pour le moment.</p>
      ) : (
        <>
